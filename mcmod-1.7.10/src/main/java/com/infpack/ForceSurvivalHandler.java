...
/**
 * 强制生存：服务器每 tick 检测（FMLCommonHandler bus 注册）。
 *
...
public class ForceSurvivalHandler {
    ...
    public void onServerTick(TickEvent.ServerTickEvent event) {
        ...
        if (ForceSurvivalConfig.isEnabled()) {
            if (server.worldServerForDimension(0) == null) {
                return;
            }
            ...
            WorldSettings.GameType worldType = server.worldServerForDimension(0).getWorldInfo().getGameType();
            boolean worldCreative = worldType == WorldSettings.GameType.CREATIVE;
            ...
            for (Object o : server.getConfigurationManager().playerEntityList) {
                EntityPlayerMP p = (EntityPlayerMP) o;
                if (p.theItemInWorldManager.getGameType() == WorldSettings.GameType.CREATIVE) {
                    p.setGameType(WorldSettings.GameType.SURVIVAL);
                    InfinitePackMod.LOG.info("[infpack] 强制生存：玩家 {} 切创造，已改回生存", p.getCommandSenderName());
                }
            }
        } else {
    ...
    private static String dimKey(WorldServer w) {
        return w.getSaveHandler().getWorldDirectoryName() + ":" + w.provider.dimensionId;
    }
    ...
    private static void ensureKeepInventoryOnAll(MinecraftServer server) {
        for (WorldServer w : server.worldServers) {
            ...
            String key = dimKey(w);
            boolean cur = w.getGameRules().getGameRuleBooleanValue("keepInventory");
            if (!ForceSurvivalConfig.hasPrevKeepInv(key)) {
