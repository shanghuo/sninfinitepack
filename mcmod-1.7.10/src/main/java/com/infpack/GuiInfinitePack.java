...
public class GuiInfinitePack extends GuiContainer {
    ...
    public void initGui() {
        super.initGui();
        this.searchField = new GuiTextField(this.fontRendererObj, SEARCH_X, SEARCH_Y, SEARCH_W, SEARCH_H);
    ...
    protected void drawGuiContainerForegroundLayer(int mouseX, int mouseY) {
        ...
        this.fontRendererObj.drawString("\u65e0\u9650\u80cc\u5305", 8, 6, 0x404040);
        ...
        if (deleteMode) {
            ...
            this.fontRendererObj.drawString(hint, SEARCH_X, STATUS_Y, 0xFF0000);
        } else {
            // 搜索框 + 空态占位提示
            this.searchField.drawTextBox();
            if (this.searchField.getText().isEmpty() && !this.searchField.isFocused()) {
                this.fontRendererObj.drawString("\u641c\u7d22", SEARCH_X + 4, SEARCH_Y + 3, 0x555555);
            }
            // 排序切换（点击循环；非默认模式橙色高亮提示已启用排序）
            String sortLabel = getSortLabel();
            this.fontRendererObj.drawString(sortLabel, getSortX(), STATUS_Y, sortMode != 0 ? 0xFF8000 : 0x404040);
        }
        ...
        int px = this.xSize - this.fontRendererObj.getStringWidth(pageText) - 6;
        this.fontRendererObj.drawString(pageText, px, STATUS_Y, deleteMode ? 0xFF0000 : 0x404040);
    }
    ...
    private void drawScreenFrame(int mouseX, int mouseY, float partialTicks) {
        ...
        if (hovered != null && hovered.slotNumber >= 0 && hovered.slotNumber < ContainerInfinitePack.ENTRY_VISIBLE) {
            ...
            if (idx >= 0 && idx < container.getStorage().size()) {
                ...
                diagIndex = idx;
                diagName = String.valueOf(Item.itemRegistry.getNameForObject(
    ...
    protected void mouseClicked(int mouseX, int mouseY, int button) {
        ...
        String sortLabel = getSortLabel();
        int sortX = getSortX();
        if (lx >= sortX - 2 && lx <= sortX + this.fontRendererObj.getStringWidth(sortLabel) + 2
