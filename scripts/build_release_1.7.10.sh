...
for f in build/libs/*.jar; do
  ...
  base=$(basename "$f" .jar)
  case "$base" in
    sninfinitepack-1.0.3)          cp "$f" /work/dist/sninfinitepack-1.0.3-mc1.7.10.jar ;;
    sninfinitepack-1.0.3-dev)      cp "$f" /work/dist/sninfinitepack-1.0.3-mc1.7.10-dev.jar ;;
    sninfinitepack-1.0.3-sources)  cp "$f" /work/dist/sninfinitepack-1.0.3-mc1.7.10-sources.jar ;;
    *)                             echo "跳过未知 jar: $base" ;;
  esac
done
