...
public class BackpackStorage {
    ...
    /** 确保背包物品有 UUID；没有则分配并写入物品 NBT（保留原有 infpack 根）。找不到背包物品时返回 null（兜底）。 */
    public static String ensureUuid(ItemStack backpack) {
        if (backpack == null) {
            return null; // 兜底：找不到背包物品时不分配（避免 NPE）
        }
    ...
    public boolean deposit(ItemStack stack, Item backpackItem, int index) {
        ...
        if (idx >= 0) {
            ...
            e.count += stack.getCount(); // 同变体：计数累加
            e.lastAccess = stamp();
            return false;
        }
        int ins = Math.max(0, Math.min(index, entries.size()));
        Entry ne = new Entry(norm, stack.getCount());
        ne.lastAccess = stamp();
        entries.add(ins, ne);
        return true;
    }
