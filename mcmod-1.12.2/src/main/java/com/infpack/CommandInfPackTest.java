...
/**
 * 服务器端自动化验证命令：/infpacktest [player] —— 1.12.2 版。
 *
...
public class CommandInfPackTest extends CommandBase {
    ...
    // 不重写 checkPermission：使用 CommandBase 默认的 OP 4 权限（避免非 OP 清空玩家背包）
    ...
    public void execute(MinecraftServer server, ICommandSender sender, String[] args) {
        ...
        if (args.length > 0) {
            ...
            player = findFirstOnlinePlayer(server);
        }
        ...
        log("目标玩家: " + (player == null ? "无" : player.getName())
    ...
    private void testStorage() {
        ...
        sword.addEnchantment(Enchantment.getEnchantmentByLocation("sharpness"), 4);
        ...
        sword2.addEnchantment(Enchantment.getEnchantmentByLocation("sharpness"), 4);
        ...
        sword3.addEnchantment(Enchantment.getEnchantmentByLocation("sharpness"), 1);
    ...
    private void testContainer(EntityPlayer player) {
        ...
        // 右键第 1 个条目（耗弓）取 1 个
        player.inventory.setItemStack(ItemStack.EMPTY);
        container.slotClick(1, 1, ClickType.PICKUP, player);
        boolean hasBow = false;
        for (int i = 0; i < player.inventory.getSizeInventory(); i++) {
            ItemStack s = player.inventory.getStackInSlot(i);
            if (!s.isEmpty() && s.getItem() == Items.BOW && s.getItemDamage() == 25) {
                hasBow = true;
            }
        }
    ...
    private int countItem(EntityPlayer p, Item item) {
        int c = 0;
    ...
    private EntityPlayer findFirstOnlinePlayer(MinecraftServer server) {
        if (server != null) {
