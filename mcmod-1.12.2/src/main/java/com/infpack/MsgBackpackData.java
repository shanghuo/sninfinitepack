...
/**
 * 服务器→客户端：下发一个背包的条目数据（存储改造）—— 1.12.2 版。
 *
...
public class MsgBackpackData implements IMessage {
    ...
    public void fromBytes(ByteBuf buf) {
        ...
        try {
            this.uuid = pb.readString(64);
            this.partIndex = pb.readVarInt();
            this.partCount = pb.readVarInt();
            int n = pb.readVarInt();
            ...
            for (int i = 0; i < n; i++) {
                itemNames[i] = pb.readString(512);
                damages[i] = pb.readInt();
                counts[i] = pb.readInt();
                lastAccesses[i] = pb.readLong();
                tags[i] = pb.readBoolean() ? pb.readCompoundTag() : null;
            }
        } catch (IOException e) {
    ...
    public void toBytes(ByteBuf buf) {
        PacketBuffer pb = new PacketBuffer(buf);
        pb.writeString(uuid);
        pb.writeVarInt(partIndex);
        pb.writeVarInt(partCount);
        int n = itemNames == null ? 0 : itemNames.length;
        pb.writeVarInt(n);
        for (int i = 0; i < n; i++) {
            pb.writeString(itemNames[i]);
            pb.writeInt(damages[i]);
            pb.writeInt(counts[i]);
            pb.writeLong(lastAccesses[i]);
            if (tags[i] != null) {
                pb.writeBoolean(true);
                pb.writeCompoundTag(tags[i]);
            } else {
